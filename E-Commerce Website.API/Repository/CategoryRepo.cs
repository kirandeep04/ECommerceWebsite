namespace E_Commerce_Website.API.Repository
{
...

    {
        ...
        public CategoryRepo(OganiContext context, IMemoryCache cache, CacheManager<Category> cacheManager) : base(context)
        {
            _context = context;
            _cache = cache;
        ...
        public override async Task<Category?> GetById(int id)
        {
            ...
            var Entityitem = CachedEntity.FirstOrDefault( g => g.Categoryid == id);
            if (CachedEntity != null)
            {
                return Entityitem;
            }
            var entity = _context.Categories.AsNoTracking().FirstOrDefault(e => e.Categoryid == id);
            if (entity != null)
            {
        ...
        public override async Task Update(int id, Category entity)
        {
            ...
            {                // Update the entity properties with the new values
                ...
                if (cachedCategory != null)
                {
                    ...
                    var index = cachedCategory.FindIndex(e => e.Categoryid == id);
                    if (index != -1)
        ...
        public override async Task Add(Category category)
        {
            ...
            else
            {
                ...
                _cache.Set("CategoriesCache", new List<Category>(), new MemoryCacheEntryOptions
                {
                    AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(10),
                    SlidingExpiration = TimeSpan.FromMinutes(1)
                });
